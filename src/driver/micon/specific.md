...
- 制御計算は `cmd_vel` から左右スラスタのDuty比を生成し、各ESP32向けへ個別に送る。
- `kinematics` が出した各スラスタの duty 指令を受けて、ESP32 へ 1 ESC ずつ送る。
- ESP32へ送る指令値は `std_msgs/msg/UInt16` を1つ。
...
### D. 検証
- `colcon build --packages-select bms thruster_driver kinematics` でビルド確認。
