...
class TestStaleAndGates:
    ...
    def test_velocity_bearing_clip_keeps_speed_on_right_to_forward_sector(self):
        velocity = np.array([0.0, 10.0, 0.2])
        clipped = tracking_glue.clip_velocity_bearing(
            velocity, 0.0, -np.pi / 2.0, 0.0)
        np.testing.assert_allclose(clipped, [10.0, 0.0, 0.2], atol=1e-12)
        np.testing.assert_allclose(velocity, [0.0, 10.0, 0.2], atol=1e-12)
    ...
    def test_velocity_bearing_clip_handles_zero_across_pi_wrap(self):
        ...
        clipped = tracking_glue.clip_velocity_bearing(
            velocity, route_yaw, -np.pi / 2.0, 0.0)
