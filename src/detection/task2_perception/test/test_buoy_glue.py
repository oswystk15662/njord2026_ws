...
def test_route_side_accepts_seven_metre_width_only_between_gps5_and_gps6():
    start, end = np.array([0.0, 0.0]), np.array([20.0, 0.0])
    assert buoy_glue.classify_route_side(
        np.array([10.0, 6.9]), start, end, 3.5, 3.5) == buoy_glue.RED
    assert buoy_glue.classify_route_side(
        np.array([10.0, -6.9]), start, end, 3.5, 3.5) == buoy_glue.GREEN
    assert buoy_glue.classify_route_side(
        np.array([10.0, 0.0]), start, end, 3.5, 3.5) is None
    assert buoy_glue.classify_route_side(
        np.array([10.0, 7.1]), start, end, 3.5, 3.5) is None
    assert buoy_glue.classify_route_side(
        np.array([-0.1, 3.5]), start, end, 3.5, 3.5) is None
