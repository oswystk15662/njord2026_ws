...
def generate_launch_description():
    ...
    # Task1-1: GPS-follow / bathymetry survey experiment (issue #45).
    # Same layered startup order as task*_sim.launch.py, but the SENSOR layer
    # drives real hardware drivers (WIT IMU, UM982 GNSS, real thruster CAN
    # transport) instead of the simulation dynamics/noise nodes, and
    # localization comes from GLIM (see robot/launch/localization.launch.py)
    # instead of a synthetic TF publisher.
    ...
    # robot_state_publisher + UM982 static TF + GLIM + local/global EKF + navsat_transform
    ...
    nav2_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            "params_file": os.path.join(pkg_robot, "config", "nav2_params_humble.yaml"),
    ...
    startup_message = LogInfo(
        msg="========== Task1-1 (GPS Follow / Depth Survey) Bringup Started ==========")
