...
def test_task1_pass_through_state_is_scoped_to_each_follow_path_goal():
    source = (Path(_LAUNCH_DIR).parents[0] / "src" / "pass_through_goal_checker.cpp").read_text()
    assert "NavigateThroughPoses may replace the FollowPath goal without resetting" in source
    assert "goal_pose.position.x != last_goal_x_" in source
    assert "entered_radius_ = false;" in source
