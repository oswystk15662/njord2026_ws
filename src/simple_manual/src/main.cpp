...
public:
	...
	: Node("serial_writer")
	{
		...
		sub_thrust_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
			"/thrust_newton", 10, std::bind(&SerialWriter::thrust_cb, this, std::placeholders::_1));
		...
		timer_ = this->create_wall_timer(std::chrono::milliseconds(50), std::bind(&SerialWriter::timer_cb, this));
...
private:
	...
	{ std::lock_guard<std::mutex> lk(mutex_); flags_.yellow = msg->data; }
	void green_cb(const std_msgs::msg::Bool::SharedPtr msg)
	{ std::lock_guard<std::mutex> lk(mutex_); flags_.green = msg->data; }
	...
	int open_serial(const std::string &dev, int baud)
	{
		...
		// set non-blocking to blocking for writes
	...
	struct Flags { bool emg=false; bool red=false; bool yellow=false; bool green=false; } flags_;
	std::array<float,4> thrust_{{0,0,0,0}};
	std::mutex mutex_;
	int fd_{-1};
